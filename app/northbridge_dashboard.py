...
# =============================================================================
# Database  : CUST360_NORTHBRIDGE_DATABASE
# Schema    : STREAMLIT (app host)
# Data      : GOLD dynamic tables
#               DIM_CUSTOMER · DIM_BRANCH · DIM_PRODUCT
...
with st.sidebar:
    ...
    segments = run_query(f"SELECT DISTINCT CUSTOMER_SEGMENT FROM {GOLD}.DIM_CUSTOMER ORDER BY 1")
    ...
    regions = run_query(f"SELECT DISTINCT REGION FROM {GOLD}.DIM_BRANCH ORDER BY 1")
...
st.caption("Enterprise risk analytics and portfolio performance across all business lines.")
st.divider()
...
with tab1:
    ...
    kpi = run_query(f"""
        SELECT
            ...
            SUM(a.CURRENT_BALANCE)                                      AS TOTAL_AUM,
            COUNT(DISTINCT l.LOAN_ID)                                   AS TOTAL_LOANS,
            SUM(l.LOAN_AMOUNT)                                          AS TOTAL_LOAN_BOOK,
            ...
            ROUND(
                ...
                / NULLIF(SUM(l.LOAN_AMOUNT), 0) * 100, 2)              AS NPL_RATIO_PCT,
            COUNT(DISTINCT CASE WHEN t.STATUS = 'Completed'
                THEN t.TRANSACTION_ID END)                              AS COMPLETED_TXNS,
            SUM(CASE WHEN t.STATUS = 'Completed' THEN t.AMOUNT ELSE 0 END) AS TOTAL_TXN_VOLUME
        FROM {GOLD}.DIM_CUSTOMER c
        LEFT JOIN {GOLD}.FACT_ACCOUNT_BALANCES a  ON c.CUSTOMER_ID = a.CUSTOMER_ID
        LEFT JOIN {GOLD}.FACT_LOANS l             ON c.CUSTOMER_ID = l.CUSTOMER_ID
        LEFT JOIN {GOLD}.FACT_TRANSACTIONS t      ON c.CUSTOMER_ID = t.CUSTOMER_ID
    ...
    col1.metric("Total Customers",   f"{int(k['TOTAL_CUSTOMERS']):,}")
    col2.metric("Total AUM",         f"${k['TOTAL_AUM']:,.0f}")
    col3.metric("Total Loan Book",   f"${k['TOTAL_LOAN_BOOK']:,.0f}")
    col4.metric("Avg Credit Score",  f"{k['AVG_CREDIT_SCORE']:.0f}")
    col5.metric("NPL Ratio",         f"{k['NPL_RATIO_PCT']:.1f}%")
    ...
    with col_a:
        fig = px.bar(
            seg_df, x="CUSTOMER_SEGMENT", y="TOTAL_BALANCE",
            color="CUSTOMER_SEGMENT", text_auto=".2s",
            title="Total AUM by Customer Segment",
            labels={"TOTAL_BALANCE": "Balance (USD)", "CUSTOMER_SEGMENT": "Segment"},
        )
    ...
    with col_b:
        fig = px.scatter(
            ...
            size="CUSTOMER_COUNT", color="CUSTOMER_SEGMENT",
            title="Income vs Credit Score by Segment",
            labels={"AVG_INCOME": "Avg Annual Income", "AVG_CREDIT_SCORE": "Avg Credit Score"},
            size_max=60,
        )
        st.plotly_chart(fig, use_container_width=True)

    # Regional AUM treemap
    reg_df = run_query(f"""
        SELECT
        ...
        LEFT JOIN {GOLD}.DIM_CUSTOMER c           ON b.BRANCH_ID = c.PRIMARY_BRANCH_ID
        LEFT JOIN {GOLD}.FACT_ACCOUNT_BALANCES a  ON c.CUSTOMER_ID = a.CUSTOMER_ID
...
with tab2:
    ...
    with col1:
        fig = px.histogram(
            cust_df, x="ANNUAL_INCOME", nbins=20,
            color="INCOME_TIER",
    ...
    with col2:
        fig = px.histogram(
            cust_df, x="CREDIT_SCORE", nbins=15,
            color="CREDIT_TIER",
    ...
    with col4:
        fig = px.pie(
            ...
            names="EMPLOYMENT_STATUS", values="count",
            title="Employment Status Mix", hole=0.4,
        )
        st.plotly_chart(fig, use_container_width=True)
    ...
    st.dataframe(
        cust_df[[
            "CUSTOMER_ID", "FULL_NAME", "CUSTOMER_SEGMENT", "CUSTOMER_TIER",
            "ANNUAL_INCOME", "CREDIT_SCORE", "CREDIT_TIER",
            "RISK_RATING", "KYC_STATUS", "CITY", "STATE", "BRANCH_ID",
...
with tab3:
    ...
    loan_df = run_query(f"""
        SELECT
            ...
            ROUND(
                ...
                / NULLIF(SUM(l.LOAN_AMOUNT), 0) * 100, 2)         AS AVG_PAYDOWN_PCT,
            SUM(CASE WHEN l.LOAN_STATUS IN ('Defaulted','Delinquent')
                THEN 1 ELSE 0 END)                                 AS AT_RISK_COUNT
        FROM {GOLD}.FACT_LOANS l
        JOIN {GOLD}.DIM_CUSTOMER c ON l.CUSTOMER_ID = c.CUSTOMER_ID
        WHERE c.CUSTOMER_SEGMENT IN ('{seg_filter}')
        GROUP BY 1, 2
        ORDER BY 1, 2
    """)
    ...
    col1.metric("Total Loans",     f"{int(loan_df['LOAN_COUNT'].sum()):,}")
    col2.metric("Total Disbursed", f"${loan_df['TOTAL_LOAN_AMOUNT'].sum():,.0f}")
    col3.metric("At-Risk Loans",   f"{int(loan_df['AT_RISK_COUNT'].sum()):,}")
    st.divider()

    col_a, col_b = st.columns(2)
    with col_a:
        by_type = loan_df.groupby("LOAN_TYPE").agg(
            TOTAL_LOAN_AMOUNT=("TOTAL_LOAN_AMOUNT", "sum"),
            LOAN_COUNT=("LOAN_COUNT", "sum"),
        ).reset_index()
        fig = px.bar(
            by_type, x="LOAN_TYPE", y="TOTAL_LOAN_AMOUNT",
            color="LOAN_TYPE", text_auto=".2s",
    ...
    with col_b:
        ...
        fig = px.pie(
            by_status, names="LOAN_STATUS", values="LOAN_COUNT",
            title="Loan Status Distribution", hole=0.35,
            color="LOAN_STATUS",
            color_discrete_map={
                "Active":      "#2ecc71",
                "Closed":      "#3498db",
    ...
    fig = px.scatter(
        loan_detail,
        x="INTEREST_RATE", y="LOAN_AMOUNT",
        color="LOAN_STATUS", size="OUTSTANDING_BALANCE",
        facet_col="LOAN_TYPE", facet_col_wrap=2,
        title="Loan Amount vs Interest Rate (bubble = Outstanding Balance)",
        labels={"INTEREST_RATE": "Rate (%)", "LOAN_AMOUNT": "Loan Amount (USD)"},
...
with tab4:
    ...
    monthly_agg = txn_df.groupby(["YEAR", "MONTH_NUM", "MONTH_NAME"]).agg(
        TXN_COUNT=("TXN_COUNT", "sum"),
        TOTAL_AMOUNT=("TOTAL_AMOUNT", "sum"),
    ).reset_index().sort_values(["YEAR", "MONTH_NUM"])
    monthly_agg["PERIOD"] = monthly_agg["MONTH_NAME"] + " " + monthly_agg["YEAR"].astype(str)

    fig = go.Figure()
    fig.add_trace(go.Bar(
        x=monthly_agg["PERIOD"], y=monthly_agg["TOTAL_AMOUNT"],
        name="Volume ($)", marker_color="#3498db", opacity=0.7, yaxis="y",
    ))
    fig.add_trace(go.Scatter(
        x=monthly_agg["PERIOD"], y=monthly_agg["TXN_COUNT"],
        name="Count", mode="lines+markers", marker_color="#e74c3c", yaxis="y2",
    ))
    fig.update_layout(
    ...
    col1, col2 = st.columns(2)
    with col1:
        by_type = txn_df.groupby("TRANSACTION_TYPE")["TXN_COUNT"].sum().reset_index()
        fig = px.bar(
            by_type.sort_values("TXN_COUNT", ascending=True),
            x="TXN_COUNT", y="TRANSACTION_TYPE",
    ...
    with col2:
        ...
        fig = px.pie(
            by_channel, names="CHANNEL", values="TOTAL_AMOUNT",
            title="Transaction Volume by Channel", hole=0.35,
        )
        st.plotly_chart(fig, use_container_width=True)

    failed = txn_df.groupby(["YEAR", "MONTH_NUM", "MONTH_NAME"])["FAILED_COUNT"].sum().reset_index()
    failed = failed.sort_values(["YEAR", "MONTH_NUM"])
    failed["PERIOD"] = failed["MONTH_NAME"] + " " + failed["YEAR"].astype(str)
    fig = px.area(
        failed, x="PERIOD", y="FAILED_COUNT",
...
with tab5:
    ...
    risk_df = run_query(f"""
        SELECT
            ...
            SUM(CASE WHEN l.LOAN_STATUS IN ('Defaulted','Delinquent')
                THEN l.OUTSTANDING_BALANCE ELSE 0 END)         AS AT_RISK_EXPOSURE
        FROM {GOLD}.DIM_CUSTOMER c
        LEFT JOIN {GOLD}.FACT_LOANS l ON c.CUSTOMER_ID = l.CUSTOMER_ID
    ...
    col1.metric("Total Exposure",        f"${risk_df['TOTAL_EXPOSURE'].sum():,.0f}")
    col2.metric("At-Risk Exposure",      f"${risk_df['AT_RISK_EXPOSURE'].sum():,.0f}")
    col3.metric("Flagged KYC",           f"{int(risk_df[risk_df['KYC_STATUS']=='Flagged']['CUSTOMER_COUNT'].sum()):,}")
    col4.metric("High-Risk Customers",   f"{int(risk_df[risk_df['RISK_RATING']=='High']['CUSTOMER_COUNT'].sum()):,}")
    st.divider()

    col_a, col_b = st.columns(2)
    with col_a:
        risk_sum = risk_df.groupby("RISK_RATING").agg(
            CUSTOMER_COUNT=("CUSTOMER_COUNT", "sum"),
            TOTAL_EXPOSURE=("TOTAL_EXPOSURE", "sum"),
            AT_RISK_EXPOSURE=("AT_RISK_EXPOSURE", "sum"),
        ).reset_index()
        fig = px.bar(
            risk_sum, x="RISK_RATING", y="TOTAL_EXPOSURE",
            color="RISK_RATING",
    ...
    with col_b:
        ...
        fig = px.pie(
            kyc_df, names="KYC_STATUS", values="CUSTOMER_COUNT",
            title="KYC Status Distribution", hole=0.35,
            color="KYC_STATUS",
            color_discrete_map={
                "Verified": "#2ecc71",
                "Pending":  "#f39c12",
    ...
    # Credit tier × risk rating heatmap
    heatmap_df = risk_df.pivot_table(
        index="CREDIT_TIER", columns="RISK_RATING",
        values="CUSTOMER_COUNT", aggfunc="sum", fill_value=0,
    ).reset_index()
    fig = px.imshow(
    ...
    """)
    fig = px.histogram(
        aml_df, x="AML_RISK_SCORE", nbins=20,
        color="RISK_RATING",
        color_discrete_map={"Low": "#2ecc71", "Medium": "#f39c12", "High": "#e74c3c"},
